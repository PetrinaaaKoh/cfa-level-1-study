# 📚 CFA Level I Study Materials - User Guide

Welcome to the CFA Level I Exam Study Platform!

---

## 🌐 **Access the Live Study Site**

### **Click here to start studying:**
# 👉 https://PetrinaaaKoh.github.io/cfa-level-1-study/ 👈

*(Or copy and paste this URL into your browser)*

---

## 📖 **What's Included**

This free study platform covers **all 10 CFA Level I topics**:

| # | Topic | Exam Weight |
|---|-------|-------------|
| 1 | Ethical and Professional Standards | 15-20% |
| 2 | Quantitative Methods | 8-12% |
| 3 | Economics | 8-12% |
| 4 | Financial Statement Analysis | 13-17% |
| 5 | Corporate Finance | 8-12% |
| 6 | Equity Investments | 10-12% |
| 7 | Fixed Income | 10-12% |
| 8 | Derivatives | 5-8% |
| 9 | Alternative Investments | 5-8% |
| 10 | Portfolio Management | 5-8% |

---

## ✨ **Features**

- ✅ **84+ Practice Questions** with detailed explanations
- ✅ **Formula Reference Cards** for quantitative topics
- ✅ **Progress Tracking** - automatically saves your completion
- ✅ **Search Function** - find any topic quickly
- ✅ **Mobile-Friendly** - study on any device
- ✅ **100% Free** - no login required
- ✅ **Works Offline** - after first load

---

## 🚀 **How to Use**

### **Step 1: Open the Study Site**
Click the link above or go to:https://PetrinaaaKoh.github.io/cfa-level-1-study/


### **Step 2: Select a Topic**
- Click any topic from the left sidebar
- Start with Ethics (highest exam weight at 15-20%)

### **Step 3: Study the Content**
- Read the key concepts
- Review formulas (where applicable)
- Study the key points

### **Step 4: Practice Questions**
- Scroll down to practice questions
- Select your answer
- Click "Submit Answer"
- Read the explanation
- Click "Next Question"

### **Step 5: Track Your Progress**
- Click "Mark as Complete" when finished
- Your progress saves automatically
- Check your overall progress at the top

---

## 💡 **Study Tips**

1. **Start with Ethics** - It's 15-20% of the exam!
2. **Practice Daily** - Even 30 minutes helps
3. **Review Explanations** - Learn from every question
4. **Track Your Progress** - Aim for 100% completion
5. **Use Search** - Find specific topics quickly

---

## 📱 **Access Anywhere**

The site works on:
- ✅ Desktop/Laptop computers
- ✅ Tablets (iPad, Android)
- ✅ Mobile phones
- ✅ Any modern web browser

**No app download needed!** Just open the link in your browser.

---

## ⚠️ **Important Notes**

- This is a **free study resource** for educational purposes
- Progress is saved in **your browser only** (clearing browser data will reset it)
- For best experience, use Chrome, Firefox, Safari, or Edge
- This material is **not affiliated with CFA Institute**

---

## 📧 **Feedback**

If you find any errors or have suggestions, please create an issue on this GitHub repository.

---

## 🙏 **Share With Others**

Know someone studying for CFA Level I? Share this link with them!

**Good luck with your studies!** 📈✨
